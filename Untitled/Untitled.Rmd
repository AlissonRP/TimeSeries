...
A Figura fornece a função de autocorrelação e autocorrelação parcial
```{r}
...
`
Pode-se notar por inspeção visual da figura, que não existem evidências da série não ser estacionária, assim como visto pelo teste ADF e pela \@ref(fig:serie).
...
```{r, fig.cap="Comportamento dos Resíduos para o Modelo Barma", fig.height = 4.7}
par(mfrow = c(2, 2)) 
...
```{r, fig.cap="Comportamento dos Resíduos para o Modelo KARMA", fig.height = 4.7}
par(mfrow = c(2, 2)) 
...
```{r, fig.height = 4.5, fig.width = 6.9}
